...
function run(msg, matches)
    ...
    if data[tostring(msg.to.id)] then
		...
		if matches[1] == 'gp' and matches[2] == 'close' then --group lock *
		...
		if matches[1] == 'gp' and matches[2] == 'open' then --group unlock *
		    if matches[3] == 'name' then
...
return {
  ...
  usage = {
    ...
    "!gp <close|open> name : Lock/unlock group name",
    "!gp <close|open> photo : Lock/unlock group photo",
    "!gp <close|open> member : Lock/unlock group member",		
    "!gp settings : Show group settings"
    },
  patterns = {
    "^[!/#](creategroup) (.*)$",
    "^[!/#](setabout) (.*)$",
